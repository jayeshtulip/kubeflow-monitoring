...
with open('p09_pipeline.yaml', 'rb') as f:
    r = requests.post(f'{base}/apis/v1beta1/pipelines/upload',
        ...
        params={'name': 'P09-RAGAS-vLLM-v3'})
print('Upload:', r.status_code)
pipeline_id = r.json().get('id', '')
print('Pipeline ID:', pipeline_id)

if not pipeline_id:
    print('Upload failed:', r.text[:200])
else:
    payload = {
        'name': 'p09-ragas-vllm-run-3',
        'pipeline_spec': {
            'pipeline_id': pipeline_id,
            'parameters': [
                {'name': 'qa_limit', 'value': '2'},
                {'name': 'run_name', 'value': 'ragas-vllm-judge-run-3'}
            ]
        },
